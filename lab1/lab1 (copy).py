...
y,edges = np.histogram(data_list, boundaries) #  note only two arguments, _ missing
...
plt.hist(x, y, facecolor='blue', alpha=0.5, histtype='bar', ec='black')
...
plt.title('Кумулята абсолютных частот')
plt.xlabel('Интервалы')
plt.ylabel('Количество значений в интервале')
plt.show()
